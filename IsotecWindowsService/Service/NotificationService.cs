...
namespace IsotecWindowsService.Service
{
    public class NotificationService : INotificationService
    {
        ...
        public NotificationService(IFilaEnvioServico filaEnvioServico,
        ...
        {
            var clientes = _clienteAppServico.GetAll().Where(x => x.FlAtivo == true).ToList();

            foreach (var cliente in clientes)
            {
                var siteCli = _siteAppServico.Get(x => x.IdCliente == cliente.IdCliente).FirstOrDefault();

                var lstPendencia = _pendenciaAppServico.PendeciaPorSite(siteCli.IdSite, cliente.IdCliente);
        ...
        public void AgendarEmail(List<Pendencia> lstPendencia)
        {
            ...
            foreach (var pendencia in lstPendencia)
            {
                ...
                filaEnvio.Assunto = pendencia.TipoPendencia == TipoPendencia.Pendencia ? "Pendência Equalit" : "Lembrete Equalit";
