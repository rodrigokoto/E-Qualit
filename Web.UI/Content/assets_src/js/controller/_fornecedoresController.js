...
APP.controller.FornecedoresController = {
	...
	setHideAndShow: function () {
		...
		$('#form-criticidade').toggle();
		$('#form-qualifica-fornecedor').toggle();
			...
			success: function (result) {
				if (result.StatusCode == 200) {
					$.each(result.AvaliacoesCriticidadePadrao, function (key, value) {
						...
						var trElements = $('<tr>');
						var tdformProdutosCriticidadeCriterio = $(`<td><input type="text" name="formProdutosCriticidadeCriterio" id="form-produtos-criticidade-criterio" class="form-control" placeholder="Critério" title="Critério" value="${value.Titulo}"></td>`);
						...
						var tdbtns = $('<td><button type="button" class="btn btn-edit-produtos-criticidade editar-color"><i class="fa fa-pencil" aria-hidden="true" data-toggle="tooltip" data-original-title="Excluir"></i><div class="tooltip fade top in" style="top: 22px; left: 1148.08px; display: block;"><div class="tooltip-arrow"></div></div></button><button type="button" class="btn btn-confirm-produtos-criticidade ativo-color"><i class="fa fa-check " aria-hidden="true" data-toggle="tooltip" data-original-title="Ativar"></i></button><button type="button" class="btn btn-del-produtos-criticidade trash-color"><i class="fa fa-trash " aria-hidden="true" data-toggle="tooltip" data-original-title="Excluir"></i></button></td>');
	...
	setNovoCriterioAvaliacaoCriticidadeFormCriticidade: function () {
		...
		this.buttonAddNovoCriterioAvaliacaoCriticidadeFormProdutos.on('click', function () {
			...
			html += '<tr>';
			html += '<td>';
			html += '<input type="text" name="formProdutosCriticidadeCriterio" id="form-produtos-criticidade-criterio" class="form-control" placeholder="Critério" />';
			html += '</td>';
			html += '<td>';
			html += '<div class="checkbox">';
			html += '<input type="checkbox" name="formProdutosCriticidadeAtivo-' + index + '" id="form-produtos-criticidade-ativo-' + index + '"  onclick="chkValue(this);" checked>';
			...
			html += '<i class="fa fa-pencil" aria-hidden="true" data-toggle="tooltip"  data-original-title="Excluir"></i>';
			html += '</button>';
			html += '<button type="button" class="btn btn-confirm-produtos-criticidade ativo-color">';
			html += '<i class="fa fa-check " aria-hidden="true" data-toggle="tooltip"  data-original-title="Ativar"></i>';
			html += '</button>';
	...
	setNovoCriterioFormQualificacaoFornecedor: function () {
		...
		this.buttonAddNovoCriterioFormQualificacaoFornecedor.on('click', function () {
			...
			var index = $('#tb-form-qualificacao-fornecedor tbody tr').size();
			...
			html += '<!-- Criterio -->';
			html += '<td>';
			html += '<input type="text" name="formQualificaFornecedorCriterio" id="form-qualifica-fornecedor-criterio" class="form-control" value="" placeholder="Critério" />';
			html += '</td>';
			html += '<!-- Ativa Item -->';
			html += '<td>';
			html += '<div class="checkbox">';
			...
			html += '<label for="form-qualifica-fornecedor-ativo-' + index + '" style="padding-left: 25px !important;">Ativar</label>';
			html += '</div>';
			html += '</td>';
			//html += '<!-- Vencimento -->';
			//html += '<td>';
			...
			//html += '<input type="text" name="formQualificaFornecedorDtVencimento" id="form-qualifica-fornecedor-dt-vencimento" class="form-control data" data-msg-required="" value="" />';
	...
	setNovoCriterioAvaliacaoFormCriterioAvaliacao: function () {
		...
		this.buttonAddNovoCriterioAvaliacaoFormCriteriosAvaliacao.on('click', function () {
			...
			html += '<td>';
			html += '<input type="text" name="formCriteriosAvaliacaoDisponibilidade" id="form-criterios-avaliacao-disponibilidade" class="form-control" value="" placeholder="Disponibilidade" />';
			html += '</td>';
			html += '<!-- Ativo -->';
			html += '<td>';
			html += '<div class="checkbox">';
	...
	setValidateFormsFornecedores: function () {
		...
		//Regras Validacao dos Formulários
		...
		this.buttonSalvar.on('click', function () {
			...
			if (validate == true) {
				...
				if ($('[name=IdFornecedor]').val() == 0) {
					...
					if (criteriosQualificacao != undefined) {

						APP.controller.FornecedoresController.saveFormAcoesQualificacoes(criteriosQualificacao);
					}
		...
		$.ajax({
			...
			success: function (result) {
				if (result.StatusCode == 200) {
					...
					window.location.replace(`AcoesFornecedores/${result.IdFornecedor}/?idProduto=${idProduto}`);
					//APP.controller.FornecedoresController.getAvaliacaoCriticidadePadrao();
				} else {
					// var texto = "";
		...
		$.ajax({
			...
			success: function (result) {
				if (result.StatusCode == 200) {
					bootbox.alert('Salvo com sucesso!');
					// $('[name=IdFornecedor]').val(result.IdFornecedor);
					// $('[name=formFornecedoresQualificacaoFornecedor]').val($('[name=formFornecedoresCadastroNome]').val());
					// APP.controller.FornecedoresController.setHideAndShowFornecedores();
					//APP.controller.FornecedoresController.getAvaliacaoCriticidadePadrao();
				}
	...
	formQualificacaoFornecedores: function () {
		...
		this.setupUploadFormQualificacaoFornecedores();
	...
	setComboResponsavelQualificacao: function () {
		...
		$.ajax({
			...
			success: function (result) {
				if (result.StatusCode == 200) {
					APP.component.SelectListCompare.init(result.Lista, $('[name=formFornecedoresQualificacaoResponsavel] option'), '#form-fornecedores-qualificacao-responsavel', 'IdUsuario', 'NmCompleto');
				}
			},
			error: function (result) {
				//bootbox.alert("Ocorreu um erro!");
	...
	setupUploadFormQualificacaoFornecedores: function (dropPE, uplPE, arquivoEvidencia) {

		var formJson = {
			'tipo': 'Upload'
		};
		APP.component.UploadFiles.setUploadFiles('/Fornecedor/UploadArquivo',
			dropPE,
			uplPE,
			arquivoEvidencia,
			30000000000,
			formJson,
			'form-fornecedores-qualificacao',
			0,
			'/Fornecedor/RemoverArquivo');

		UploadDeleteFiles('/Fornecedor/RemoverArquivo');
	...
	setComboResponsavelControlarQualificacao: function () {
		...
		$.ajax({
			...
			success: function (result) {
				if (result.StatusCode == 200) {
					APP.component.SelectListCompare.init(result.Lista, $('[name=formFornecedoresQualificacaoResponsavelControlar] option'), '#form-fornecedores-qualificacao-responsavel-controlar', 'IdUsuario', 'NmCompleto');
				}
			},
			error: function (result) {
				//bootbox.alert("Ocorreu um erro!");
	...
	getObjFormQualificacaoFornecedores: function () {
		...
		$('form[id^=form-fornecedores-qualificacao-]').each(function (key, val) {
			var obj = {
				IdCriterioQualificacao: $('[name=IdCriterioQualificacao]').val(),
				IdProduto: $('[name=IdProduto]').val(),
				Titulo: $(this).find('h2').text(),
				...
				//obj.ArquivoEvidencia = $(this).find('[name=arquivoEvidencia]').val('teste.pdf');
				ArquivoEvidencia: 'teste.pdf',
