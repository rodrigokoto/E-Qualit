...
namespace Dominio.Servico
{
    public class DocDocumentoServico : IDocDocumentoServico
    {
        ...
        public DocDocumentoServico(IDocDocumentoRepositorio documentoRepositorio,
                                   ...
                                   IAnexoRepositorio _anexoRepositorio)
        {
        ...
        private DocDocumento CriarCloneRevisadoComMaisUm(DocDocumento documentoAtual)
        {
            ...
            documentoNovo.ArquivoDocDocumentoAnexo = documentoAtual.ArquivoDocDocumentoAnexo;

            TratarAnexos(documentoNovo, documentoAtual);
        ...
        private void TratarAnexos(DocDocumento documentoNovo, DocDocumento documentoAtual)
        {
            if (documentoAtual.ArquivoDocDocumentoAnexo == null)
                return;
            ...
            foreach (var arquivo in documentoAtual.ArquivoDocDocumentoAnexo)
            {
                ...
                arquivoDocDocumentoAnexo.Anexo = new Anexo();

                arquivoDocDocumentoAnexo.Anexo.Nome = arquivo.Anexo.Nome;
                arquivoDocDocumentoAnexo.Anexo.Extensao = arquivo.Anexo.Extensao;
                arquivoDocDocumentoAnexo.Anexo.Arquivo = arquivo.Anexo.Arquivo;
                arquivoDocDocumentoAnexo.Anexo.DtAlteracao = DateTime.Now;
                arquivoDocDocumentoAnexo.Anexo.DtCriacao = DateTime.Now;

                documentoNovo.ArquivoDocDocumentoAnexo.Add(arquivoDocDocumentoAnexo);
            }
        }
        ...
        public void Valido(DocDocumento documento, ref List<string> erros)
        {
            ...
            List<string> errosRisco = new List<string>();
            documento.DocRisco.ForEach(x =>
            {

                if (string.IsNullOrWhiteSpace(x.DescricaoRegistro))
                ...
                if (x.PossuiGestaoRisco == null)
                    errosRisco.Add("Campo  necessita de ação deve ser preenchido");


                if (x.PossuiGestaoRisco == true)
                {
                    if (string.IsNullOrEmpty(x.Causa))
                        errosRisco.Add("Campo causa obrigatório");

                    if (x.IdResponsavelInicarAcaoImediata == null || x.IdResponsavelInicarAcaoImediata < 1)
                        errosRisco.Add("Campo responsável obrigatório");

                }
                else
                {
                    if (string.IsNullOrEmpty(x.DsJustificativa))
                        errosRisco.Add("Campo justificativa obrigatório");
